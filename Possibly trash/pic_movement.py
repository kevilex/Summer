...
for e in range(0, int((math.pi)*100), int((math.pi*10))):
    ...
    for i in range(0, int((math.pi)*100), int((math.pi))):
        point = Point()
        point.x = ((math.cos(i/100)*r)/5) - 0.4
        point.y = (y/2) - 0.1
        point.z = ((math.sin(i/100)*r)/3) + 0.25
...
arm.group.set_max_velocity_scaling_factor(0.1)
arm.group.set_goal_tolerance(0.005)
arm.group.set_goal_orientation_tolerance(0.1)
arm.group.set_goal_joint_tolerance(0.01)
#arm.group.set_planning_time(20)
#arm.group.set_num_planning_attempts(100)

arm.add_waypoint(-0.2, -0.2, 0.2, 0, math.pi, 0)
#arm.add_waypoint(-0.5, 0.1, 0.2, 0, 0, 0)

(plan, fraction) = arm.group.compute_cartesian_path(arm.waypoints, 0.01, 0.0, True)
#pub.publish(arm.marker)
#arm.move_endpoint(-0.15, -0.08, 0.2)
