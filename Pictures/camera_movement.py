...
rospy.init_node('pic_movement', anonymous=True)
# Creating arm object and defining variables.
arm = UR5(clear_octomap)

arm.clear_waypoints()

# Positive x-direction is the diagonal from the robot towards the table.
# The first quadrant of the coordinate system is to the left of the diagonal seen from the robot.
# It will most likely be the same as the offset, but it's here if needed.
obj_x = 0.35
obj_y = 0.0

# Offsets to mark the middle of the 'sphere'.
offset_x = 0.5
offset_y = 0
# The starting height of the arc, recommend to start at 0.25 or higher.
height = 0.3

# Scaling up the size of the arc, the default is 0.1
scale_x = 2.1
scale_y = 6
scale_z = 3

# Debugging can be done by vizualising 'Markers' in RViz.

for e in range(0, int((math.pi)*100), int((math.pi)*10)):
    y = (math.sin((e/100) + (math.pi/2)))/2
    r = math.sin(e/100)

    for i in range(0, int((math.pi)*100), int((math.pi))):
        point = Point()
        point.x = (((math.cos(i/100)*r)/10)*scale_x) + (offset_x * (-1))
        point.y = ((y/10)*scale_y) + (offset_y * -1)
        point.z = (((math.sin(i/100)*r)/10)*scale_z) + height

        if 'prev' in globals() and point != prev:

            if point.x-obj_x > 0:
                yrh = math.sqrt(abs((point.z)**2 + (point.x-(obj_x*-1))**2))
                ry = math.asin((point.x-(obj_x*-1))/yrh) + math.pi
            else:
                yrh = math.sqrt(abs((point.z)**2 + ((obj_x*-1)-point.x)**2))
                ry = -math.asin(((obj_x*-1)-point.x)/yrh) + math.pi
            
            if point.y-obj_y > 0:
                xrh = math.sqrt(abs(point.z**2 + (point.y-(obj_y*-1))**2))
                rx = math.asin((point.y-(obj_y*-1))/xrh)
            else:
                xrh = math.sqrt(abs(point.z**2 + ((obj_y*-1)-point.y)**2))
                rx =  -math.asin(((obj_y*-1)-point.y)/xrh)
